...
The eleventh and FINAL stabilization cycle of the 3-month campaign. Cycle L lands the first canonical performance + fuzz regression suite for do it. Before this cycle, the project had **zero** performance regression guards — a future contributor could (a) add heavy sync work to `build()` (e.g., a `find.byType(...)` scan, a DB read on the UI thread, a JSON parse in the hot path), (b) split `DoRepository.listAll` / `listActive` into per-do reads (the N+1 antipattern), or (c) break the immutability / runtime-type / field-preservation invariants on the pure-Dart model layer — without any test catching the regression. Cycle L closes the Cycle A audit's "Performance: zero tests" gap with 10 new tests + 1 new doc.

**New code (test-only + docs only — no production code changes):**

- `test/perf/widget_rebuild_test.dart` (NEW, +3 testWidgets) — pins the per-cycle cost of a Listenable-driven rebuild inside a MaterialApp + Provider tree. The widget tree is built ONCE outside the measurement loop (no `pumpWidget` re-mounts inside the loop — those would include FutureBuilder + DB queries and dominate the signal). The loop pushes `ValueNotifier.value = i + 1` and measures `await tester.pump()` cost. Budgets (regression-direction guard, not absolute perf — real-device release builds are 3-5× faster per Flutter's published guidance): cold mount ≤ 750 ms; single-tile rebuild ≤ 5 ms median over 100 iterations; 10-tile rebuild ≤ 25 ms median over 100 iterations. Observed at Cycle L's first run: cold mount ~262 ms; single-tile rebuild ~2 ms median; 10-tile rebuild ~10 ms median.
- `test/perf/sql_benchmark_test.dart` (NEW, +2 tests) — pins the N+1 invariant on `DoRepository.listAll` + `listActive` via a Drift `QueryExecutor` proxy (`_CountingExecutor`) wrapping `NativeDatabase.memory()` (the standard Drift test seam — delegates every method to the wrapped executor, so behavior under test is unchanged). Asserts exactly 1 SELECT for N=10 seeded habits on both methods + median ms ≤ 10 for `listActive` over 50 iterations. Observed at Cycle L's first run: `listAll` issues exactly 1 SELECT; `listActive` issues exactly 1 SELECT with median < 1 ms.
- `test/fuzz/do_model_fuzz_test.dart` (NEW, +2 tests × 1000 iterations) — fuzzes the `Do` constructor + `copyWith` invariants + `Do.validate()` exception surface contract with `Random(42)` seed (no `package:faker` per Cycle L pre-auth; `dart:math.Random(seed)` is the same RNG the production code uses for `MathProblem.next` / `MemoryGame.generate`). `Do.validate()` must throw only `DoValidationException` (never any other type); `copyWith(name: X).name == X`; runtime type preserved; `copyWith()` without args equals source. Sanity pin: at least one valid + one invalid branch observed over the 1000 iterations (catches a fuzz that exercises only one branch).
- `test/fuzz/person_model_fuzz_test.dart` (NEW, +1 test × 1000 iterations) — fuzzes `ContactPerson` + `PersonCadence` constructors + `copyWith` invariants; every `PersonCadence` subclass (`EveryNDays`, `WeeklyOn`, `MonthlyOn`, `YearlyOn`) constructs without throwing; channel swap preserves `ContactPerson.id`.
- `test/fuzz/mission_model_fuzz_test.dart` (NEW, +1 test × 1000 iterations) — fuzzes `MissionChain.from([...])` (length + order + runtime-type preserved) + `Mission.verify(TextInput('hello'))` (returns `MissionResult` without throwing; returns `MissionFailed` for the obvious input-mismatch on every subclass except `TypeMission`); `MissionChain.empty.length == 0` (the `SoftProof` default).
- `test/fuzz/consecutive_counter_fuzz_test.dart` (NEW, +1 test × 1000 iterations) — fuzzes the streak calculator: `currentStreak ≥ 0` (never negative); `longestStreak ≥ currentStreak`; deterministic across two calls with the same input log; missing days past the grace window break the streak; rest-day entries within the grace window preserve it; duplicate same-day entries do not double-count.
- `docs/v_model/performance_baseline.md` (NEW) — documents the observed baseline numbers from Cycle L's first run, the regression-direction guard rationale (real-device release builds are 3-5× faster; the budgets are for catching heavy-sync-work regressions, not absolute perf), the N+1 invariant's hard nature (a future contributor who splits `listAll` / `listActive` into per-do reads would silently regress N+1 queries, and the test fails with a clear reason), the median-vs-mean rationale (single timings swing 5-10× on CI; median is robust), and the `dart:math.Random(seed)` rationale (no `package:faker` per Cycle L pre-auth; the same RNG the production code uses). The doc's "What Cycle L does NOT cover" section defers profile-mode timings + end-to-end scroll perf + APK size to the W-13 closeout (the stabilization retrospective).

**Coverage**: no production-code coverage delta (Cycle L is pure-test + docs; the perf + fuzz tests pin the EXISTING behavior and serve as tripwires for future regressions).

**3-gate**: format clean (0 changed after auto-format on the 6 NEW test files); analyze 0 issues; 1547/1547 tests pass.

**No release APK rebuild** — Cycle L is the FINAL cycle in the v1.4-stab campaign; APK SHA1 stays at Cycle J's `25bb7fab8ce3834fbc15b0a624229f09b3e49a4d` (no production code changes — test-only + docs cycle).

**No new `<uses-permission>`, no new pubspec deps, no Drift migration, no Kotlin changes** — Cycle L is intentionally minimal-touch as the FINAL cycle in the campaign.
...
**Drift lessons (per ADR-079):** (a) **UI form is much simpler than master plan assumed** — the form renders ONLY `EveryNDays(nDays)` cadence + ONLY `ChannelDialer(phoneNumber)` channel; the 4 cadence shapes + 5 channel leaves exist in the model but not in the form; v0.2 will introduce more (per `add_person.dart:9` file header); (b) **Picker mocking is heavyweight** — `LocationPicker.show()` and `CalendarPicker.show()` both gate on `PermissionSheet.show(...)` then `showModalBottomSheet<Automation>`; stubbing either requires the full permissions + geolocator/calendar channel pair; deferred to v2.0; (c) **Edit-mode deadlock pattern is shared with add_habit** — chained `runAsync` for seed save + `_loadExisting` races with Drift's keepalive close (mirrors ADR-078 (e)); deferred.
...
**Test count: 1727 → 1737 (+10 net, exactly the plan target).** **Cumulative v1.6:** 1727 → **1737 tests** (+10 net). **APK SHA1 stays at Cycle H's `25bb7fab8ce3834fbc15b0a624229f09b3e49a4d`** — v1.6-η is tests-only; no production-code behavior change; no release APK rebuild. **No new `<uses-permission>`, no new pubspec deps, no Drift migration, no Kotlin changes.** **Out-of-scope (deferred to v2.0 + ADR-083):** `WidgetActionInvoker.dispatch` happy path with a real `WidgetService.instance` (requires Drift + a real `Do` repository + the `WidgetService.init` happy-path fakes from `widget_service_test.dart` — out-of-cycle due to the singleton-with-`_ready` setup overhead; covered indirectly by `widget_service_test.dart`'s `markDone appends the completion then re-derives` baseline); `WidgetActionInvoker.ready` future (the `Completer<void> _ready` is private and tested via the public `attach`/`resetForTesting` contract); `WidgetServiceProxy` stays at 33.3% per the ADR-071 trade-off. Cycle is the SEVENTH in the v1.6 milestone — next is **v1.6-θ (PR #75) MissionChain + sparkline + consecutive_counter + mission_result**.

**Refs:** SYS-152, ADR-083, WF-080.

## v1.6-θ — Cross-cutting invariants coverage closure (Phase 66 / SYS-153 / ADR-084 / WF-081)

The eighth cycle of the v1.6 milestone — closes cross-cutting invariant + boundary coverage gaps on 3 small-but-mission-critical Dart modules: `lib/missions/chain.dart` (the `MissionChain.totalTimeout` boundary at the empty chain + the `Iterable` contract via `iterator`/`first`/`last`/`contains`), `lib/screens/home_tile_sparkline.dart` (the `SparklineDotFilled` non-manual source-tag preservation, the 14-day window boundary at day-15, the determinism contract, the `toString` debug-representation pin), and `lib/do/consecutive_counter.dart` (the `StreakSnapshot` value-equality contract, the `CompletionLogEntry` identity-equality pin, the grace-window boundary at exactly 03:00 of the day AFTER a missed day, the `SkipBudget` → `StreakSnapshot.restDaysUsed` cross-cutting propagation). **Test-only cycle**; no production-code behavior change; hits the v1.6 pre-auth plan target of +10 tests exactly.

**Chain_api tests (2 new in `MissionChain API` group):** (1) `totalTimeout of an empty chain is Duration.zero (boundary invariant — v1.6-θ)` — both `MissionChain.empty.totalTimeout` and `MissionChain.from(<Mission>[]).totalTimeout` return `Duration.zero`; pins the `fold(Duration.zero, ...)` seed behavior; (2) `iterator yields missions in declared order (cross-cutting Iterable contract — v1.6-θ)` — a 3-mission chain `[hold, hold2, type]` iterates in that exact order via `for-in`; pins `first`/`last`/`contains` so a future "wrap as a `Set` instead" refactor fails loudly.

**Home_tile_sparkline tests (4 new in `sparklineForDo` group, with `// ---- v1.6-θ ----` banner):** (3) `SparklineDotFilled preserves non-manual source tags (v1.6-θ)` — seed `_FakeCompletionLog` with `source: 'auto'`; assert the rendered dot is `SparklineDotFilled` with `source == 'auto'`; (4) `extendedSparklineForDo(days: 14) ignores a row 15 days ago (boundary at day-15 — v1.6-θ)` — pins the 14-day window boundary; (5) `extendedSparklineForDo is deterministic across two consecutive calls with the same args (idempotency — v1.6-θ)` — element-by-element structural equality; (6) `SparklineDotFilled.toString includes day + source for debugging (debug-representation pin — v1.6-θ)` — pins both fields' presence in the rendered string without locking the format.

**Consecutive_counter tests (4 new in NEW `Cross-cutting invariants (v1.6-θ)` group):** (7) `StreakSnapshot value equality + hashCode match for identical contents (v1.6-θ)` — two `const StreakSnapshot(...)` with identical fields are `equals` AND have matching `hashCode`; single-field-different `const StreakSnapshot` is NOT equal; (8) `CompletionLogEntry value equality matches field-wise (v1.6-θ)` — `CompletionLogEntry` does NOT override `==` so identity-equality applies; pins the current behavior; (9) `grace window boundary at exactly 03:00 of the day after a missed day keeps the run alive (v1.6-θ)` — `asOf = 2026-01-16 02:59:59` (1 day after `lastCompletion = 2026-01-15`) keeps the run alive; `asOf = 2026-01-16 03:00:01` breaks the run with `brokenAt = 2026-01-16`; (10) `SkipBudget consumption propagates into StreakSnapshot.restDaysUsed (cross-cutting — v1.6-θ)` — consume 1 day from the budget; the resulting snapshot has `restDaysUsed == 1` even with a fresh 1-completion log.

**Drift lessons per ADR-084:** (a) **`const` literals with identical fields are canonicalized** — `const a = StreakSnapshot(...)` and `const b = StreakSnapshot(...)` resolve to the SAME instance; tests pin the PUBLIC contract (value-equality + matching hashCodes) WITHOUT asserting on identity; (b) **Grace window logic ONLY fires when `daysSinceLast == 1`** — an `asOf` 2+ days after the last completion is BROKEN regardless of grace window; (c) **`SparklineDotFilled.toString` is implementation-defined but MUST include both fields** — pins the two-field-presence contract WITHOUT locking the format; (d) **`completionLog.listForHabit` is called per-helper-invocation** — the determinism test verifies structural equality, NOT identity; (e) **MissionChain inherits `UnmodifiableListView`'s `iterator`** — pins declared-order iteration + `first`/`last`/`contains` so a future "wrap as a `Set` instead" refactor fails loudly (chain semantics are ordered, not set-semantic).

**Test count: 1737 → 1747 (+10 net, exactly the plan target).** **Cumulative v1.6:** 1737 → **1747 tests** (+10 net). **APK SHA1 stays at Cycle H's `25bb7fab8ce3834fbc15b0a624229f09b3e49a4d`** — v1.6-θ is tests-only; no production-code behavior change; no release APK rebuild. **No new `<uses-permission>`, no new pubspec deps, no Drift migration, no Kotlin changes.** **Out-of-scope (deferred to v2.0 + ADR-084):** `SparklineDot.future(day)` factory direct emission (the helper's 7-day/14-day window always ends at `asOf`'s local-midnight); `MissionChain.from(<Mission>[])` equality with `MissionChain.empty` (the cross-cutting equality pin via non-canonical pair is the next-best coverage); `StreakSnapshot` `copyWith` (no such method exists); `CompletionLogEntry` value-equality override (would be a behavior change). Cycle is the EIGHTH in the v1.6 milestone — next is **v1.6-ι (PR #76) db.dart + migrations + permission_observer + main.dart**.
...
The ninth cycle of the v1.6 milestone — closes coverage gaps on 4 critical-path modules: `lib/services/db.dart` (the Drift singleton's state-after-failure paths — `init()` propagates platform-channel errors via both the `Future` return AND `ready.future`, AND keeps `_db` unassigned so the getter still throws StateError; `closeForTesting()` resets `_db` to null so the getter throws until the next `init()`), `lib/services/db/migrations/v1_to_v2.dart` (the v0.2 column adds — `category` / `color_seed` / `icon_name` / `paused_until_millis` / `end_hour` / `end_minute` / `target_hours` on habits, `paused_until_millis` on people; the new `events` / `person_groups` / `person_group_members` tables; existing rows survive the migration with v0.2 default values applied), `lib/services/permission_lifecycle_observer.dart` (the cold-start `_coldStartSeen` short-circuit is synchronous — NO microtask is scheduled by the first `resumed` event; sequential non-cold-start resumed events each produce one `PermissionService.refresh()` fire — three back-to-back resumed events produce ≥3 fires; the `triggerRefreshForTest()` test hook drives the `ReliabilityService.StateError` catch branch deterministically), and `lib/main.dart` `DoItApp` (the first-launch gate, MultiProvider, theme + `AppLocalizations` delegate + supportedLocales + `onUnknownRoute` + title). **Test-only cycle**; no production-code behavior change.

**DB_singleton tests (2 new):** `init_failure_surfaces_via_ready.future_and_db_stays_uninitialized` + `closeForTesting_resets_db_accessibility_until_next_init`. **Migration_test tests (3 new using `_V2OnlyDatabase`):** v0.2 columns added, new tables created, existing rows preserved with defaults. **Permission_observer tests (3 new):** cold-start short-circuit, sequential refreshes, `triggerRefreshForTest` StateError catch. **Main_test tests (10 NEW):** first-launch gate (4) + wiring pins (6).

**Test count: 1747 → 1765 (+18 net, exactly the plan target).** **Cumulative v1.6:** 1747 → **1765 tests** (+18 net). **APK SHA1 stays at Cycle H's `25bb7fab8ce3834fbc15b0a624229f09b3e49a4d`** — v1.6-ι is tests-only; no production-code behavior change; no release APK rebuild. **No new `<uses-permission>`, no new pubspec deps, no Drift migration, no Kotlin changes.** **Out-of-scope (deferred to v2.0 + ADR-085):** full migration chain v1→v5 on a v1 fixture (latent `events.automations_json` duplicate-column conflict would require idempotent createTable guards or check-then-add rewrite — both out-of-scope for tests-only); `main()` end-to-end integration test (would require massive platform-channel mock fixture); `DoItApp` MultiProvider consumer lookup (pinned at `MaterialApp` attribute level instead).

Cycle is the NINTH in the v1.6 milestone — next is **v1.6-κ (PR #77) TemplateLibrary.seedBuiltIns wiring + automationsJson restore**.
...
**Test count: 1765 → 1773 (+8 net, exactly the plan target).** **Cumulative v1.6:** 1765 → **1773 tests** (+8 net). **APK SHA1 stays at Cycle H's `25bb7fab8ce3834fbc15b0a624229f09b3e49a4d`** — v1.6-κ is comment-cleanup only (3 stale `Phase B PR 2` TODOs retired); no production-code behavior change; no release APK rebuild. **No new `<uses-permission>`, no new pubspec deps, no Drift migration, no Kotlin changes.** **Out-of-scope (deferred to v2.0 + ADR-086):** the v1.0 restore flow that wipes the templates table and forces a re-seed (no such flow exists today; the `builtInOnly` short-circuit at `seedBuiltins:399-400` would silently no-op if even one built-in survived); `main()` end-to-end integration test (would require massive platform-channel mock fixture); v2.0 codemod to retire other vestigial `Phase B PR 2` TODOs if any survive.

Cycle is the TENTH in the v1.6 milestone — next is **v1.6-λ (PR #78) Doc cleanups (0 tests)**.

**Refs:** SYS-154, ADR-085, WF-082.
...
- **Batch 7 — Existing v1.6-β regressions extended (1 test):** preserves the existing duplicate-name + rest-day + icon-name + category coverage as regression-protectors.

**Test count: 1773 → 1786 (+13 net — under plan's +18 target by 5; the menu→dialog post-save path was replaced with simpler round-trip coverage).** Coverage: `lib/screens/add_habit.dart` ~76% → ~85% (+9 pp). APK SHA1 stays at H's `25bb7fab8ce3834fbc15b0a624229f09b3e49a4d` (no production-code change). **No new `<uses-permission>`, no new pubspec deps, no Drift migration, no Kotlin changes.** Full suite shows 1786/1786 pass.
...
**Out-of-scope (deferred to v2.0 + ADR-088):** full `_saveAsTemplate` dialog flow with menu + dialog Save button; `_pickAnchorTarget` empty-list snack `'No other dos to anchor on.'` (line 717); `_pickInterval` decrement clamp beyond 1; `_pickNth` max=5 clamp.

Cycle is the **FIRST** in the v1.7 milestone — v1.7 milestone OPENED. **Refs:** SYS-157, ADR-088, WF-085.
...
- **Batch 2 — Column-overload isolation (1 test):** hand-write 2 rows with distinct `dayOfMonth` values (15 for MonthlyOn, 4 for YearlyOn day); assert each subtype round-trips with its OWN `dayOfMonth`. Covers the column overload at `:66` + `_parseCadence` at `:131-144`.
- **Batch 3 — Null-default read-path pins (4 tests):** hand-write 4 rows with the relevant nullable column OMITTED (Drift default null); assert the 4 `?? 1` fallbacks at `:134, 136, 138, 140` decode the default (`EveryNDays(1)` + `WeeklyOn(1)` + `MonthlyOn(1)` + `YearlyOn(1, 1)`).
...
- **Batch 1 — `_pickLead` dialog (5 tests):** pin all 4 reachable buckets of `_leadLabel(int m)` + the Cancel branch.
- **Batch 2 — `_applyPayload` defensive branches (6 tests):** pin the 5 typed guards at `:110-118`. The 3 `Event.validate()` throw branches are UNREACHABLE from form input — v1.7-γ does NOT write tests for them.
- **Batch 3 — `_saveAsTemplate` envelope variants (1 test):** pin the `'annually'` branch of the envelope ternary at `:343-344`.
...
**Out-of-scope (deferred to v2.0 + ADR-090):** A lead-time slider (NOT in v0.1 form; deferred to v2.0 per the file's own structure); a 3-leaf `RadioListTile` recurrence group (v0.1 supports only 2 leaves); a recurring-end-date picker (NOT in v0.1; the `annually` leaf is open-ended); a wider type cast in `_applyPayload` (a future refactor that removes the typed guards is a BREAKING change).

Cycle is the **THIRD** in the v1.7 milestone (3/9 cycles shipped). **Refs:** SYS-159, ADR-090, WF-087.

## v1.7-δ — Spanish ARB verbatim-pin coverage closure (Phase 72 / SYS-160 / ADR-091 / WF-088)

The **FOURTH cycle of the v1.7 milestone** — pins the post-v1.3 Spanish ARB catalog (the 129 `String get` entries in `lib/l10n/gen/app_localizations_es.dart`) as the **regression guard for the future translator pass** (B2 in the 3-month launch plan per `~/.claude/plans/here-now-i-hvae-enumerated-reddy.md`). **No production-code change.** Tests-only cycle.

**Scope:** EXTEND `test/l10n/locale_render_test.dart` (+20 tests, 9 baseline → 29 total).
...
- **Batch 5 — Settings theme + anchor + permission status (8 tests):** pin the v0.1 settings-screen surface — `settingsThemeDark` + `settingsThemeLight` + `settingsThemeSystem` + `settingsAnchorFirstUnlock` + `settingsAnchorEither` + `permissionStatusGranted` + `permissionStatusNotAsked` + `permissionStatusDenied`.

**Test count: 1812 → 1832 (+20 net — exactly matches the v1.7 pre-auth plan target).** Cumulative v1.7 progress: 1773 → 1832 (+59 net across α + β + γ + δ). APK SHA1 stays at H's `25bb7fab8ce3834fbc15b0a624229f09b3e49a4d` (no production-code change). **No new `<uses-permission>`, no new pubspec deps, no Drift migration, no Kotlin changes.** Full suite shows 1832/1832 pass.
...
Cycle is the **FOURTH** in the v1.7 milestone (4/9 cycles shipped). **Refs:** SYS-160, ADR-091, WF-088.
...
**Test count: 1832 → 1842 (+10 net — exactly matches the v1.7 pre-auth plan target).** Cumulative v1.7 progress: 1773 → 1842 (+69 net across α + β + γ + δ + ε). APK SHA1 stays at H's `25bb7fab8ce3834fbc15b0a624229f09b3e49a4d` (no production-code change). **No new `<uses-permission>`, no new pubspec deps, no Drift migration, no Kotlin changes.** Full suite shows 1842/1842 pass.
...
Cycle is the **FIFTH** in the v1.7 milestone (5/9 cycles shipped). **Refs:** SYS-161, ADR-092, WF-089.
...
**Test count: 1842 → 1850 (+8 net — exactly matches the v1.7 pre-auth plan target).** Cumulative v1.7 progress: 1773 → 1850 (+77 net across α + β + γ + δ + ε + ζ). APK SHA1 stays at H's `25bb7fab8ce3834fbc15b0a624229f09b3e49a4d` (no production-code change). **No new `<uses-permission>`, no new pubspec deps, no Drift migration, no Kotlin changes.** Full suite shows 1850/1850 pass.
...
## PR4 of 15 (2026-07-07)

Extracted `AppPalette` primitive (`lib/ui/app_palette.dart` — new 4th file in the `lib/ui/` design-system layer, ~108 lines) with 3 `static` semantic-color helpers (`iconMuted`, `mutedTileBackground`, `mutedPillDecoration`) + 5 canonical constants. Migrated 3 color-palette-misuse sites to the canonical pattern: `widget_config_screen.dart:156` `Colors.grey` → `AppPalette.iconMuted(context)`; `home.dart:_TileIcon:1270` `color.withValues(alpha: 0.20)` → `AppPalette.mutedTileBackground(context, color)`; `do_anchor_paused_badge.dart:68-75` inline `BoxDecoration` → `AppPalette.mutedPillDecoration(context, color)`. All 3 migrations preserve visuals byte-for-byte (the 5 canonical constants preserve the original 0.20 / 0.15 / 0.5 / 0.5 / 8 values exactly). 11 new tests in `test/ui/app_palette_test.dart` (4 groups: iconMuted [3], mutedPillDecoration [5], mutedTileBackground [2], static accessors [1]). Test count: 1899 → 1910 (+11 net; EXACT match with plan). 3-gate green (`dart format` clean + `flutter analyze --fatal-infos lib test` 0 issues — 15 deprecation warnings caught + fixed via `.r/.g/.b/.a` per ADR-101 lessons (b)+(c) + `flutter test` 1910/1910 pass). No manifest / pubspec / Drift / Kotlin changes. V-Model artifacts: SYS-170 + ADR-101 + WF-098 + 5 doc rows.
...
## v1.8-08 — PR8 of 15 (AppSnack snackbar wrapper primitive)

C4 form patterns batch 2. NEW `lib/ui/app_snack.dart` (~85 LOC) — `AppSnack` class with private constructor + 2 `static` methods `showError` (M3 `errorContainer` background + `onErrorContainer` text) and `showInfo` (M3 default `inverseSurface` styling). 6 migration sites across 2 screens: `add_habit.dart` 3 sites (`_showSnack` helper body, pre-modal "No other dos to anchor on.", captured-messenger "Delete failed. Please try again." with new `if (!mounted) return;` guard) + `add_event.dart` 3 sites (pre-modal "Give the event a name first.", post-save "Template saved", `TemplateValidationException` "Template validation failed: ..."). 10 new widget tests in `test/ui/app_snack_test.dart` (5 groups: showError [4: fires ScaffoldMessenger + applies errorContainer + applies onErrorContainer + renders message verbatim], showInfo [3: fires ScaffoldMessenger + uses M3 default styling + renders message verbatim], semantic distinction [1: same message produces different visual treatment], theme integration [1: light theme], static accessor [1]). Test count 2000 → 2010 (+10 net). 3-gate green. APK discipline anchor maintained (no `AndroidManifest.xml` / pubspec / Drift / Kotlin changes). SYS-174 / ADR-105 / WF-102.

## v1.8-09 — PR9 of 15 (EmptyState + LoadingView primitives)

C5 empty/loading states batch 1. NEW `lib/ui/empty_state.dart` (~100 LOC) — `class EmptyState extends StatelessWidget` with `title` (required, M3 `titleLarge`) + optional `message` (M3 `bodyMedium`), `icon` (rendered at `Sizing.huge` in `colorScheme.outline` for the calm muted tone), and `action` (`Widget?` slot for a future CTA). Both text widgets have `textAlign: TextAlign.center` so the empty state is visually balanced on any width. NEW `lib/ui/loading_view.dart` (~55 LOC) — `class LoadingView extends StatelessWidget` with default full-area spinner (36dp / 4dp stroke, M3 "comfortable" sizing) + `LoadingView.inline()` named constructor for in-row use (20dp / 2dp stroke, M3 "small" sizing). The constructor takes optional `size` and `strokeWidth` parameters that override the defaults. The `Center + SizedBox + CircularProgressIndicator` composition is the canonical M3 loading affordance; the primitive does NOT add a `Padding` or `Container` wrapper (the caller controls layout). 6 migration sites across 5 screens: `home.dart` 2 sites (private `_EmptyState` widget removed + `FutureBuilder` `Center(child: CircularProgressIndicator())` → `const LoadingView()`), `stats.dart` 1 site (private `_EmptyView` removed — title + message + icon variant), `templates.dart` 1 site (private `_EmptyView` removed — title-only quiet variant), `events.dart` 1 site (FutureBuilder CircularProgressIndicator → `LoadingView()`), `person_groups.dart` 1 site (FutureBuilder CircularProgressIndicator → `LoadingView()`). 21 new widget tests in `test/ui/empty_state_test.dart` (10 tests, 5 groups: required title [2: title verbatim + centered], optional message [2: message verbatim + omits when null], optional icon [2: renders + omits], optional action [2: renders + omits], visual treatment [2: textAlign center + Spacing.lg padding], theme integration [1: light theme]) + `test/ui/loading_view_test.dart` (11 tests, 5 groups: full-area default [3: renders CircularProgressIndicator + centers + 36dp/4dp defaults], custom size + stroke [2: custom + non-default non-inline], in-line variant [2: 20dp/2dp defaults + custom], tree composition [2: stacks independently + no Padding/Container wrapper], theme integration [1: light theme]). Test count 2010 → 2031 (+21 net; -1 vs +22 plan target — the test count target included 1 extra test for the `homeEmptyBody` localization that lands in PR-D, not PR9). Net -68 LOC across 5 screens (89 deletions, 21 additions) — the consolidation is real: the 3 private empty-state widgets + 3 inline CircularProgressIndicator centers collapse into 2 primitives. 3-gate green (1 `unused_import` for `package:doit/theme/app_theme.dart` in `loading_view.dart` caught + removed by the `flutter analyze` lint; 0 `prefer_single_quotes` issues). APK discipline anchor maintained (no `AndroidManifest.xml` / pubspec / Drift / Kotlin changes). SYS-184 / ADR-115 / WF-111.

## v1.8-10 — PR10 of 15 (ErrorView primitive)

C5 empty/loading/error states batch 2. NEW `lib/ui/error_view.dart` (~75 LOC) — `class ErrorView extends StatelessWidget` with `message` (required, M3 `bodyLarge`) + `onRetry` (required, `VoidCallback`) + optional `retryLabel` (defaults to `l.homeRetryButton` from the canonical localization; resolved via `AppLocalizations.of(context)`). The `FilledButton` retry carries the canonical `ValueKey('error.retry')` identifier so widget tests can find it across all 3 migration sites without per-site `ValueKey`s (the old `home.retry` / `stats.retry` / `templates.retry` keys are replaced by this one). The visual structure mirrors `EmptyState`: `Center > Padding(EdgeInsets.all(Spacing.lg)) > Column(MainAxisSize.min, [Text(message), Spacing.md gap, FilledButton])`. 3 migration sites across 3 screens: `home.dart` 1 site (private `_ErrorView` widget removed — `_refresh` callback preserved), `stats.dart` 1 site (private `_ErrorView` widget removed + the hardcoded literal `'Could not load stats.'` was promoted to the new `message:` parameter + the hardcoded `'Retry'` was replaced with `l.homeRetryButton` — eliminates 2 hardcoded English strings), `templates.dart` 1 site (private `_ErrorView` widget removed). 11 new widget tests in `test/ui/error_view_test.dart` across 6 groups: required message [2: renders verbatim + Center ancestor], default retry label [1: defaults to `l.homeRetryButton`], custom retry label [1: honors override + default hidden], retry button affordance [2: `ValueKey('error.retry')` discoverable + tap fires `onRetry`], visual treatment [2: `Spacing.lg` padding (scoped to `find.byType(ErrorView)` to disambiguate from Scaffold padding) + message `bodyLarge` fontSize 16], theme integration [2: light theme renders correctly + `FilledButton` type is theme-driven], message + retry label independence [1: both overrides coexist]. Test count 2031 → 2042 (+11 net; matches +11 plan target exactly). Net -72 LOC across 3 screens (79 deletions, 7 additions). The `retryLabel` defaults to `l.homeRetryButton` (canonical "Retry" string) — the localization helper is mandatory because every site reads the same English string today, and PR10 introduces the canonical helper without expanding the ARB surface. 3-gate green. APK discipline anchor maintained (no `AndroidManifest.xml` / pubspec / Drift / Kotlin changes). SYS-185 / ADR-116 / WF-112.

## v1.8-11 — PR11 of 15 (MissionFailedView primitive + C9-1 a11y fix)

C5 mission error UX. NEW `lib/ui/mission_failed_view.dart` (~75 LOC) — `class MissionFailedView extends StatelessWidget` rendering an `AlertDialog` (M3 canonical modal — the canonical "your action did not succeed" surface) with 3 strings read from `AppLocalizations.of(context)`: `missionFailedTitle` ("Mission failed"), `missionFailedBody` ("The mission did not complete. Your streak stays unchanged."), `missionFailedDismiss` ("OK"). The dismiss CTA is a `FilledButton` carrying the canonical `ValueKey('mission_failed.dismiss')` identifier. The dialog body is wrapped in `Semantics(liveRegion: true, label: '$title. $body')` so TalkBack announces the failure as soon as the dialog opens — this is the **C9-1 a11y fix** bundled with PR11 (the canonical TalkBack label pattern per `.claude/rules/lib-screens.md:34-39`). The dialog has exactly 1 CTA — the user can't retry from this dialog (the mission screen is already popping); "OK" simply acknowledges and dismisses. The primitive is presented via `showDialog<void>(context: ..., builder: (ctx) => MissionFailedView(onDismiss: () => Navigator.of(ctx).pop()))`; the caller controls the dialog lifecycle via the required `onDismiss: VoidCallback`. 3 migration sites across 3 screens: `mission_math.dart` 1 site (3rd-wrong auto-fail shows MissionFailedView before the screen-pop + `Text.semanticsLabel: 'Math problem, ${_problem.a} ${_opSymbol(_problem.op)} ${_problem.b}. Type the answer.'` on the problem card per `lib-screens.md:38`), `mission_type.dart` 1 site (same pattern + `Text.semanticsLabel: 'Type the phrase, ${widget.mission.expectedPhrase}, to confirm.'` on the phrase card per `lib-screens.md:37`), `mission_launcher.dart` 1 site (both `mission-aborted` and `chain-failed` paths route through new `_showMissionFailedAndDismiss()` helper — eliminates the silent pop-with-null UX gap so the user gets visible "your streak is intact" feedback). 3 new ARB keys (`missionFailedTitle` / `missionFailedBody` / `missionFailedDismiss`) added in en + es (Spanish mirror: "Misión fallida" / "La misión no se completó. Tu racha se mantiene sin cambios." / "Aceptar"). 10 new widget tests in `test/ui/mission_failed_view_test.dart` across 5 groups: required content [2: title verbatim + body verbatim], dismiss CTA [3: dismiss CTA text + `ValueKey('mission_failed.dismiss')` discoverable + tap fires `onDismiss`], a11y [1: body wrapped in `Semantics(liveRegion: true)` with the concatenated label — C9-1 fix coverage], dialog shape [2: `AlertDialog` type + light theme renders], body independence [2: title + body + CTA all render together + Semantics label combines title + body verbatim]. Test count 2042 → 2052 (+10 net; -1 vs +11 plan target — the 3 mission screen widget tests (`mission_math_test.dart` / `mission_type_test.dart` / `mission_launcher_test.dart`) were updated to wire `AppLocalizations.localizationsDelegates` into the `MaterialApp` test wrapper and dismiss the dialog via `tester.tap(find.byKey(const ValueKey('mission_failed.dismiss')))` before the final pop assertion — the migration coverage was absorbed by the existing flow tests rather than added as new tests). 3-gate green (1 `await_in_wrong_context` lint in math/type `_submit()` — fix: mark `void _submit() async`; 1 `unintended_html_in_doc_comment` lint in `mission_failed_view.dart` docstring — fix: rephrase to avoid `<void>` angle brackets; 1 `prefer_const_declarations` lint in `mission_launcher.dart` `_showMissionFailedAndDismiss()` — fix: `const reason = 'mission-failed'`). APK discipline anchor maintained (no `AndroidManifest.xml` / pubspec / Drift / Kotlin changes). SYS-186 / ADR-117 / WF-113.

## v1.8-pr-d — PR-D (Rest-day caption wording flip + localized empty-state body)

Retention sprint PR-D. **2 small inconsistencies from the v1.4 era bundled into one PR** (each too small to warrant its own PR).

**(1) Rest-day caption wording flip** ([lib/screens/home.dart:1573](lib/screens/home.dart#L1573)): changed `l.homeTileBudgetRemaining(budget.remaining, budget.limit)` → `l.homeTileBudgetUsed(budget.used, budget.limit)`. The new framing ("Used X of Y this month" vs the old "X/Y rest days left") matches the StreakService "Done 12 of 30 this month" framing — both express "consumed budget" rather than "remaining stash", encouraging users to think of rest days as something they actively spend (matching v0.1's design intent per ADR-009 / WF-006). The legacy `homeTileBudgetRemaining` ARB key is kept in the catalog with a back-compat `@description` note (no callers remain).

**(2) Localized empty-state body**: the home screen's `EmptyState(title: l.homeEmptyTitle, icon: Icons.bolt_outlined)` call site at [home.dart:257](lib/screens/home.dart#L257) gains a `message: l.homeEmptyBody` parameter — the body was hardcoded English `'Tap the + to add a do or a person.'` since v0.1 (an oversight from the original implementation; PR9 / SYS-184 introduced the `message:` slot but did not migrate the body). Spanish mirror: "Toca + para añadir una tarea o una persona."

2 EDIT sites (both in `lib/screens/home.dart`): `_BudgetCaption.build` normal-bucket branch + the `EmptyState` call site at `_HomeScreenState.build`. 4 NEW ARB keys: `homeTileBudgetUsed` (en: "Used {used} of {limit} this month"; es: "Usados {used} de {limit} este mes") + `homeEmptyBody` (en: "Tap the + to add a do or a person."; es: "Toca + para añadir una tarea o una persona."). 1 EDIT ARB description: `homeTileBudgetRemaining` description now states the legacy/back-compat status (no callers remain).

5 NEW tests in `test/screens/home_tile_budget_caption_test.dart` (English ARB round-trip / Spanish mirror / empty-state body en / empty-state body es / `EmptyState` primitive title-→-message-with-gap render). 4 UPDATED tests: 2 `locale_render_test.dart` empty-state tests now also pin `l.homeEmptyBody` (in en + es), 2 `home_test.dart` caption-render tests renamed to reference v1.8-pr-d / SYS-193 + assertion text updated to "Used X of Y this month".

Test count 2084 → 2089 (+5 net; +9 if you count the 4 existing-test extensions). 3-gate green. APK discipline anchor maintained: no `AndroidManifest.xml` change, no new pubspec dep, no Drift migration, no Kotlin change. SYS-193 / ADR-124 / WF-120.

## v1.8-pr-c — PR-C (Passphrase input on the Restore-from-Backup screen)

Retention sprint PR-C. **Unblock v2/v3 encrypted-backup restore UX** — v2/v3 backups produced via the `doit.tour` post-onboarding pass are encrypted with Argon2id at `kBackupFormatVersion = 3` per ADR-045 / SYS-115 and could NOT be restored from the UI before this PR. The service's `BackupFormatException` ('Backup is encrypted (v$version); a passphrase is required.') surfaced as a raw error Card on the Restore screen. The fix is purely screen-layer: `BackupService.importFrom` at [lib/services/backup_service.dart:190](lib/services/backup_service.dart#L190) already accepts the `{String? passphrase}` named parameter.

EDIT [lib/screens/settings_restore.dart](lib/screens/settings_restore.dart) — added `TextEditingController _passphraseController` (disposed in `dispose()`, cleared on every successful re-pick — a new backup may not share the passphrase with the old one) + `bool _obscurePassphrase = true` (default). Render a new `Card` between the picked-path Card and the Replace button with a `TextField` (`obscureText: _obscurePassphrase`, `autocorrect: false`, `enableSuggestions: false`, `suffixIcon: IconButton` carrying the canonical `ValueKey('settings_restore.passphrase.toggle')` for show/hide with `Icons.visibility_outlined` / `Icons.visibility_off_outlined`) + the localized `settingsRestoreEncryptedBackupHint` below. In `_restore()`: `final typed = _passphraseController.text; final passphrase = typed.isEmpty ? null : typed;` then `importFrom(File(path), passphrase: passphrase)`. Empty input maps to `null` (preserves the v1 plain-JSON contract — no passphrase). The `Decryption failed` service-layer error is mapped to a localized `settingsRestoreWrongPassphraseError` ("Wrong passphrase — could not decrypt this backup." / "Frase de contraseña incorrecta — no se pudo descifrar la copia."); all other `BackupFormatException` messages (e.g., `Could not read file: ...`) pass through verbatim. The body Column is wrapped in `SingleChildScrollView(physics: const ClampingScrollPhysics())` to handle the new Card on small viewports (the screen was always vertically constrained in real usage, but the test viewport is smaller than a phone — surfaced as a `RenderFlex` overflow by 56 pixels in 2 of the new tests).

6 NEW ARB keys (en + es): `settingsRestorePassphraseLabel` ("Backup passphrase (optional)" / "Frase de contraseña de la copia (opcional)") + `settingsRestorePassphraseHint` ("Required for encrypted (.json v2 or v3) backups" / "Necesaria para copias cifradas (v2 o v3)") + `settingsRestorePassphraseShowCta` ("Show" / "Mostrar") + `settingsRestorePassphraseHideCta` ("Hide" / "Ocultar") + `settingsRestoreWrongPassphraseError` ("Wrong passphrase — could not decrypt this backup." / "Frase de contraseña incorrecta — no se pudo descifrar la copia.") + `settingsRestoreEncryptedBackupHint` ("This backup is encrypted. Enter the passphrase used when it was exported." / "Esta copia está cifrada. Introduce la frase de contraseña usada al exportarla.").

1 NEW test file `test/screens/settings_restore_passphrase_test.dart` (10 tests in 5 groups: ARB round-trip en / ARB round-trip es / initial render after pick x2 / interaction x3 / integration x3). 1 EDIT test file `test/screens/settings_restore_test.dart` — switched `_wrap()` from a bare `MaterialApp` to `support.localizedApp(...)` (per the PR11 lesson; 4 pre-existing tests would otherwise crash on the generated `AppLocalizations.of(context)!` assertion because the new localized strings render post-pick).

Test count 2089 → 2099 (+10 net; matches the plan's "+10" target exactly). 3-gate green. APK discipline anchor maintained: no `AndroidManifest.xml` change, no new pubspec dep, no Drift migration, no Kotlin change. Drift lessons in ADR-123: **(a)** the service throws a single `BackupFormatException` per failure mode — a future hardening is a sealed `BackupFailure` exception-code taxonomy (deferred to v2.1); **(b)** the existing test wrap (`MaterialApp(...)` direct) is not future-proof — adding any localized string to a post-pick render path now requires the `localizedApp(...)` helper from `test/support/localized_app.dart`. SYS-192 / ADR-123 / WF-119.

## v1.8-pr-b — PR-B (Post-onboarding CoachMark tour + home empty-state "Show me around" CTA)

Retention sprint PR-B. **Close the "no guided tour of the FAB" + "no CTA on the home empty state" product gaps** surfaced by the v1.8 UI consolidation sprint's user-testing pass. After onboarding finishes, the user lands on a Home screen that is intentionally empty (no dos yet) with the FAB as the only call-to-action. The FAB is small (56dp), located bottom-right, and not obvious to a first-time user — and there is no other affordance hinting at what the app does. The fix is a 2-step coach-mark tour that walks the user through the FAB target and the `AddHabitScreen` schedule picker, plus a `PrimaryButton` "Show me around" CTA on the home empty state for users who dismissed the tour.

NEW [lib/ui/coach_mark.dart](lib/ui/coach_mark.dart) (~280 LOC; 17th `lib/ui/` file) — `class CoachMarkOverlay extends StatefulWidget` rendering a full-screen `Stack` with a dimmed scrim (`colorScheme.scrim` with 56% `withValues(alpha:)` — `Color.opacity` is deprecated in Flutter 3.31+, per the PR4 lesson) + a `CustomPaint` ring around the target's `RenderBox` (the rect is computed in `addPostFrameCallback` so the lookup is post-first-frame; `RenderBox.localToGlobal(Offset.zero)` gives the screen-space top-left, the size gives the rect) + a `Card` callout (`Positioned` below the target with a 12dp gap + a `Column` of title + body + two text buttons in a `Row`). The bubble carries `Semantics(liveRegion: true, label: tourBubbleAriaLabel(title: ..., body: ...))` so TalkBack announces the bubble when it mounts. Canonical `ValueKey('tour.skip')` / `ValueKey('tour.next')` for the Skip / Next CTAs (the latter's label flips to `l.tourDone` on the last step). API: `CoachMarkOverlay({required TourStep step, required VoidCallback onNext, required VoidCallback onSkip})`. Static helper `CoachMarkController.start(BuildContext context, List<TourStep> steps)` walks the queue: each step pushes a `MaterialPageRoute<bool>` (route returns `true` to advance, `false` to abort) and the controller decides whether to push the next step or end the tour based on the popped value + whether more steps remain. `class TourStep` — required `targetKey: GlobalKey` + `title: String` + `body: String` + optional `onAdvance: Future<void> Function()?` (async hook fired BEFORE the route pops — used by the home FAB step to navigate to `AddHabitScreen` BEFORE the schedule-picker step's `findRenderObject` rect lookup runs).

EDIT [lib/services/settings_service.dart](lib/services/settings_service.dart) — added `static const String _kTourSeenKey = 'doit.tour.seen'` + `ValueNotifier<bool> tourSeen = ValueNotifier<bool>(false)` (private field, public notifier — same pattern as `firstLaunchDone`) + `final _tourSeen = _prefs.getBool(_kTourSeenKey) ?? false;` in `init()` to hydrate the notifier + `Future<void> markTourSeen() async { _tourSeen = true; await _prefs.setBool(_kTourSeenKey, true); }` setter + `tourSeen.value = false; _tourSeen = false;` in `resetForTesting()`.

EDIT [lib/screens/home.dart](lib/screens/home.dart) — added `final GlobalKey _fabKey = GlobalKey(debugLabel: 'home.fab');` to `_HomeScreenState` + threaded it into the new `_AddFab(fabKey:)` param (defaulting to `ValueKey('home.fab')`). The home empty state is wrapped in a `ValueListenableBuilder<bool>(valueListenable: SettingsService.instance.tourSeen, builder: (_, tourSeen, __) { if (tourSeen) return _quietEmptyState(...); return _EmptyState(..., action: PrimaryButton(key: const ValueKey('home.empty.tour_cta'), label: Text(l.homeEmptyTourCta), onPressed: () => _startTour(context))); })` — the CTA renders only when `tourSeen == false` and re-triggers the tour from the empty state. New private `_startTour(BuildContext)` method builds the 2-step queue: step 1 (`_fabKey` + `tourStep1Title` / `tourStep1Body`) has `onAdvance` that pushes `AddHabitScreen`; step 2 (`GlobalObjectKey('tour.schedule_picker')` + `tourStep2Title` / `tourStep2Body`) waits for the route push to settle before its rect lookup.

EDIT [lib/screens/add_habit.dart](lib/screens/add_habit.dart) — schedule-type `SegmentedButton<String>` (around line 379) carries `key: const GlobalObjectKey('tour.schedule_picker')`. `GlobalObjectKey` is the canonical cross-screen shared key (vs `ValueKey` which is identity-equal by widget tree path; the schedule picker is on a different route, so `GlobalObjectKey` is required).

9 NEW ARB keys (en + es): `homeEmptyTourCta` ("Show me around" / "Muéstrame cómo funciona") + `tourStep1Title` ("Add your first do" / "Añade tu primer do") + `tourStep1Body` ("Tap the + button to create a new do — anything you want to remember to do." / "Pulsa el botón + para crear un nuevo do: cualquier cosa que quieras recordar.") + `tourStep2Title` ("Pick a schedule" / "Elige un horario") + `tourStep2Body` ("Choose how often this do should remind you. You can change it later." / "Elige cada cuánto este do debe recordártelo. Puedes cambiarlo más tarde.") + `tourNext` ("Next" / "Siguiente") + `tourSkip` ("Skip tour" / "Saltar tour") + `tourDone` ("Done" / "Listo") + `tourBubbleAriaLabel` ("Tour step: {title}. {body}" / "Paso del tour: {title}. {body}" — uses `placeholders` metadata so the generated signature is `String Function(String title, String body)`, NOT a 2-arg curry).

3 NEW test files:
- [test/ui/coach_mark_test.dart](test/ui/coach_mark_test.dart) — 9 tests in 4 groups (required content [3: title verbatim / body verbatim / target rect computed from RenderBox after first frame], CTAs [3: Next key + label / Skip key + label / last step flips label to `tourDone`], a11y liveRegion [1: Semantics wrapper exposes `tourBubbleAriaLabel` to TalkBack], multi-step controller [3: walks N steps in order / `onAdvance` hook fires before route pop / Skip returns early with `false` and exits cleanly]).
- [test/services/settings_service_tour_flag_test.dart](test/services/settings_service_tour_flag_test.dart) — 4 tests (default `tourSeen == false` after `init()` / `markTourSeen()` persists `true` via `_prefs.setBool` / restart reads `true` from SharedPreferences / `resetForTesting()` clears the notifier to `false` and resets the completer).
- [test/screens/home_empty_state_tour_cta_test.dart](test/screens/home_empty_state_tour_cta_test.dart) — 4 tests (CTA renders when `tourSeen == false` / CTA hidden when `tourSeen == true` / Spanish locale renders `l.homeEmptyTourCta` localized / tapping CTA mounts `CoachMarkOverlay` route).

Test count 2099 → 2116 (+17 net; +6 over the plan's "+11" target — the multi-step controller group + the CTAs group got more tests than planned). 3-gate green. APK discipline anchor maintained (no `AndroidManifest.xml` / pubspec / Drift / Kotlin changes). Drift lessons codified in ADR-122: **(a)** `PrimaryButton.label` is `Widget` (not `String`) — wrap localized text in `Text(l.foo)` per the M3 `FilledButton.icon` contract (the v1.8-cyc-01 / PR1 primitive cannot take a bare String); **(b)** the `use_build_context_synchronously` lint tracks `BuildContext.mounted` (Flutter 3.7+), NOT `State.mounted` — using the State's `mounted` getter does NOT satisfy the lint; **(c)** `GlobalObjectKey` is the canonical shared key for cross-screen widget targets (vs `ValueKey` which is identity-equal by widget tree path and breaks on route boundaries). SYS-191 / ADR-122 / WF-118.

## v1.8-pr-e1 — PR-E1 (`scheduled_messages` Drift table for one-shot contact reminders; data-layer only)

NEW Drift table `scheduled_messages` (9 columns: `id` / `person_id` (nullable) / `channel_tag` / `channel_handle` / `message_body` (nullable) / `fire_at_millis` / `status` (SQL default `'pending'`) / `created_at_millis` / `fired_at_millis` (nullable)) added via a v5 → v6 migration (`lib/services/db/migrations/v5_to_v6.dart`). Bumped `kCurrentSchemaVersion` from 5 to 6. The migration only creates the new table — no existing row or column is touched; the on-create path (`m.createAll()`) creates the table in lockstep with the rest of the schema on fresh installs. The data-layer split (this PR is data-only — no UI, no service, no ARB, no platform change) is canonical per `.claude/rules/lib-services.md` §"When changing this folder": "A DB migration is its own PR. Do not bundle with a feature." The wire + UI land in PR-E2 (`url_launcher` integration + `PersonChannel.launch()` + Person-tile "Send message" CTA + Schedule-a-message screen + notification tap-to-launch path; ~+25 tests). 1 NEW migration file (`lib/services/db/migrations/v5_to_v6.dart`, ~40 LOC, single `m.createTable(db.scheduledMessages)` call) + 1 EDIT Drift-table file (`lib/services/db/tables.dart` — added `class ScheduledMessages extends Table` with `@DataClassName('ScheduledMessageRow')` + 9 columns; the `channelTag` text column accepts one of the 5 `PersonChannel` variant tags: 'dialer' | 'whatsapp' | 'telegram' | 'signal' | 'sms'; `channelHandle` is denormalized from `People.handle` at schedule time so the launch path works after a person delete; `status` defaults to `'pending'` at the SQL level via `withDefault(const Constant('pending'))`; lifecycle values are 'pending' | 'fired' | 'dismissed' | 'cancelled' stored as plain strings to match the existing `People.channel` / `Habits.proofMode` / `Events.recurrence` text-discriminator convention — the type system would force a migration every time a new value is added) + 1 EDIT schema file (`lib/services/db/schema.dart` — bumped `kCurrentSchemaVersion` 5 → 6; added `ScheduledMessages` to `@DriftDatabase(tables: [...])` list; added `if (from < 6) await migrateV5ToV6(m, this);` to `MigrationStrategy.onUpgrade`; added the v6 entry to the file-header version history) + REGEN `lib/services/db/schema.g.dart` via `dart run build_runner build` (emits `$ScheduledMessagesTable extends ScheduledMessages` + `class ScheduledMessageRow extends DataClass` + `ScheduledMessagesCompanion.insert(...)` factory). 4 EDIT pre-existing test files (`migration_test.dart` + `migration_v2_to_v3_test.dart` + `migration_v3_to_v4_test.dart` + `migration_v4_to_v5_test.dart` — version-pin assertions bumped from `5` to `kCurrentSchemaVersion` / `6`; the new `migration_v3_to_v4_test.dart` + `migration_v4_to_v5_test.dart` + `migration_v5_to_v6_test.dart` use `kCurrentSchemaVersion` to be future-proof against the next bump). 1 NEW test file (`test/db/migration_v5_to_v6_test.dart`, 7 tests in 3 groups: schemaVersion/table-exists/columns [3] / SQL-default-via-customInsert + nullable-NULL [2] / full-row-round-trip + lifecycle-storable [2]). Test count 2116 → 2123 (+7 net). 3-gate green. **APK discipline anchor MAINTAINED** — no `AndroidManifest.xml` change, no `pubspec.yaml` change, no Kotlin change; the Drift migration is the entirety of the change surface.

**Drift lessons in ADR-125:** (a) `isNull` is ambiguous between `package:drift` and `package:flutter_test` (Drift exports `isNull` as a query predicate; `flutter_test` exports `isNull` as a matcher; when both are imported, the analyzer flags `ambiguous_import`; the clean fix is `import 'package:drift/drift.dart' hide isNull;` to keep the matcher, or remove the drift import entirely if it's unused). (b) `withDefault` does NOT make the Drift-generated Dart constructor parameter optional (the SQL `DEFAULT 'pending'` clause applies to a raw `INSERT` that omits the column, but the generated `ScheduledMessageRow(...)` constructor still has `required this.status` — the workaround in tests is `ScheduledMessagesCompanion.insert(...)`, where the `Companion.insert` factory makes every column optional with `Value.absent()` by default, so omitting `status:` exercises the SQL default). (c) Bumping the schema version ripples into 5 pre-existing test files (every test that pins the schema version as a literal — `expect(kCurrentSchemaVersion, 5)` or `expect(db.schemaVersion, 5)` — needs to be updated to either `kCurrentSchemaVersion` (future-proofs against the next bump) or the new literal — the new `migration_v5_to_v6_test.dart` and the renamed `migration_v3_to_v4_test.dart` + `migration_v4_to_v5_test.dart` use `kCurrentSchemaVersion` to avoid the next drift; the older `migration_test.dart` + `migration_v2_to_v3_test.dart` use the literal `6` for historical specificity).

SYS-194 / ADR-125 / WF-121.
