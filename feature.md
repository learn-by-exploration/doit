...
Cycle C (`feat/v1.4-stab-C-fullscreen-android14`) is the
immediate next cycle and will be planned in its own plan-mode
session. Cycle C will harden the full-screen launch path for
Android 14+ API 34+ (BUG-003 — the `USE_FULL_SCREEN_INTENT`
permission probe + UI flow + MissionScreen launch verification
on API 34+). The plan for Cycle C will reference the audit
findings in `docs/v_model/stabilization_roadmap.md` §2 to
confirm the priority sequencing — BUG-003 is P1 (reliability)
and is the third cycle in the stabilization campaign per
`docs/v_model/plan.md` Milestone 12 §"Month 1".
