...
| **Calendar service (v1.0/Phase E)** | `CalendarService` singleton — thin platform adapter for `CalendarContract.Instances` via a native `ContentObserver` (ADR-023). Production wires a `_MethodChannelCalendarSource` that talks to the `doit/calendar` method channel; tests wire a `ScriptedCalendarSource`. The Kotlin side registers a `ContentObserver` on `CalendarContract.Instances` and emits transitions (`started` / `ended` / `reminderFired` / `busyStatusChanged`) to Dart via the method channel. The reactive `ContentObserver` model replaces the 5-minute poll that the v1.0e.1 draft planned. `CalendarPicker` widget exposes a one-tap picker on the Add screens' Routines section. | `lib/services/calendar_service.dart`, `android/app/src/main/kotlin/com/doit/CalendarChannel.kt`, `lib/widgets/calendar_picker.dart` |
| **CallInterceptor service (v1.0/Phase F)** | `CallInterceptor` Kotlin `CallScreeningService` (ADR-019). The interceptor reads the Japan routine config from `SharedPreferences` on every call (the Dart isolate may be asleep). `lib/services/call_interceptor.dart` exposes `setJapanRoutine` / `setCallPattern` / `setSilentModeOverride` over the `doit/call_interceptor` method channel; PR 2 adds `isCallScreeningRoleHeld` + `requestCallScreeningRole` (`RoleManager.createRequestRoleIntent(ROLE_CALL_SCREENING)`, ADR-019 follow-up). `JapanRoutineConfig` persists the config; `lib/screens/add_routine.dart` renders the UX; `SettingsService.japanRoutine` is a `ValueNotifier<JapanRoutineConfig?>`. The routine gracefully no-ops when the role is not held (the notification still fires; the silent-mode override is skipped). | `android/app/src/main/kotlin/com/doit/CallInterceptor.kt`, `lib/services/call_interceptor.dart`, `lib/services/japan_routine_config.dart`, `lib/screens/add_routine.dart` |
| **Device-state PR 2 dispatch + debug screen (v1.0/Phase D PR 2)** | `RoutineExecutor._onDeviceState` subscribes to `DeviceStateService.instance.snapshots` and dispatches `TriggerDeviceState` matchers (all 7 trigger shapes: `charging` / `batteryRange` / `bluetoothDevice` / `wifiSsid` / `headphones` / `ringerMode` / `foregroundApp`). Per ADR-022, the executor never polls; the reactive `DeviceStateChannel.kt` broadcasts are the only source. The Settings → Triggers debug screen surfaces a live dashboard of the 7 properties (battery %, charging state, BT connected devices, current Wi-Fi SSID, headphones, ringer mode, foreground app). The screen has a 60-second poll slot for the dashboard; the executor never reads it. | `lib/routines/routine_executor.dart` (`_onDeviceState` arm), `lib/screens/settings.dart` (Triggers debug section) |
| **Calling-contact wire (v1.8-pr-e2 / PR-E2 / SYS-195..198)** | End-to-end wire for the v2.0 "call/message this contact" + "schedule a one-shot message" features. Layer 1 — `PersonChannel.launch({String? body}) → Uri`: each of the 5 sealed `PersonChannel` variants (`ChannelDialer` / `ChannelWhatsApp` / `ChannelTelegram` / `ChannelSignal` / `ChannelSms`) builds a channel-specific URI; the body is URL-encoded via `Uri.encodeComponent` and appended as `?text=` (WA / Telegram / SMS) or omitted (dialer / Signal — both open the chat/dialer with no pre-fill). Layer 2 — `url_launcher: ^6.3.x` package (first new pubspec dep since v1.7-ζ / ADR-093) wraps `Intent.ACTION_VIEW` / `ACTION_DIAL` / `ACTION_SENDTO`. Layer 3 — Android 11+ package visibility: a `<queries><package>` block in `AndroidManifest.xml` declares `com.whatsapp` / `org.telegram.messenger` / `org.thoughtcrime.securesms` + `<intent>` blocks for `ACTION_SENDTO sms:` and `ACTION_DIAL tel:`. Layer 4 — Kotlin notification builder branch in `MainActivity.buildReminderNotification`: when the fired alarm resolves to a `ScheduledMessage` (vs. a `HabitReminder`), the `PendingIntent` targets the channel URI directly (`ACTION_VIEW` for `https://wa.me/...` / `https://t.me/...` / `https://signal.me/...`; `ACTION_SENDTO sms:` for SMS; `ACTION_DIAL tel:` for dialer) with `FLAG_ACTIVITY_NEW_TASK`. `PersonChannel.fromTag(String, String)` factory throws `ArgumentError` for unknown tags — forward-compat graceful degradation. The wire is **launch-only** (no `http(s)://` data calls): the CI SYS-026 grep continues to reject `package:http` + `Uri.http(s)(` even though `https://wa.me` etc. appear in source as `Uri.parse(...)` launch targets. APK discipline anchor bumped per ADR-126: test count + 3-gate green + no Drift-schema / Kotlin-API changes; pubspec + manifest `<queries>` + Kotlin builder branches are explicitly allowed. | `lib/people/person.dart` (`launch()` + `fromTag()`), `lib/services/scheduled_message_repository.dart`, `lib/services/reminder_service.dart` (`scheduleScheduledMessage` + `onAlarmFire`), `android/app/src/main/AndroidManifest.xml` (`<queries>`), `android/app/src/main/kotlin/com/doit/MainActivity.kt` (`buildScheduledMessageNotification`) |
| **ScheduledMessage service (v1.8-pr-e2 / PR-E2 / SYS-196)** | Singleton service that wraps the PR-E1 `scheduled_messages` Drift table behind a small pure-Dart `ScheduledMessage` model. CRUD: `save(ScheduledMessage)` (insert-or-replace), `insert(...)` (new row, optional `status:` exercises the SQL `withDefault('pending')`), `markFired(id, firedAt)`, `cancel(id)`, `deleteById(id)`, `deleteForPerson(personId)`. Queries: `getById(id)`, `listAll()`, `listPending()` (used by `BootReceiver` `rescheduleAll` to rebuild the alarm table), `pendingFor(personId)`. Status enum `ScheduledMessageStatus` (3 values: `pending` / `fired` / `cancelled`) with `fromWire()` forward-compat fallback. Pure-Dart value object — no Flutter imports, no `DateTime.now()` (callers pass the reference time). | `lib/services/scheduled_message_repository.dart` |
