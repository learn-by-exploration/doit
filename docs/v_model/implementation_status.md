...
| **v1.1e — offline `LocationMapPreview` for `LocationPicker`** | `<v1.1e SHA>` | ✓ | 822 / 822 (807 prior + 15 new — 11 preview widget/helper tests + 4 picker tests) | `lib/widgets/location_map_preview.dart` (SYS-084 / ADR-028). Pure `CustomPaint` widget with stylised grid + pin + geofence ring; no `flutter_map`, no `INTERNET`. Equirectangular projection helpers exposed for tests. `LocationPicker` mounts the preview between the lat/lon fields and the "Use current location" button; lat/lon `TextFormField`s gain `onChanged: (_) => setState(() {})` so the pin follows typed coordinates. `test/widgets/location_map_preview_test.dart` (11 tests). `test/widgets/location_picker_test.dart` adds 3 new tests + a scroll-up drag for the On exit path (the preview's added height made the Save button sit at the bottom-sheet's scroll-detector overlap). v1.2 candidate: swap the `CustomPaint` body for `flutter_map` + cached tiles + the `INTERNET` permission. |
