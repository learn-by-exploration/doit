...
| v0.5a — rename to "do it" | _pending v0.5a commit_ | ✓ | 377 / 377 + v0.5a pin tests in `release_signing_test.dart` (TBD at v0.5a) | App-level rename from "Streak" to "do it". `applicationId` / `namespace` → `com.doit.package`. Dart package `name:` → `doit`. Directory `streak/` → `doit/`. Kotlin tree `com/common_games/streak/` → `com/doit/package/`. `MethodChannel('doit/reminders')`, notification channel `doit.reminders`, WorkManager task `doit.backup.nightly`. Version bump `0.4.0+5` → `0.5.0+6`. `StreakCalculator` / `StreakService` / `StreakSnapshot` / `StreakConfig` / `streak_calculator.dart` / `streak_service.dart` stay (feature identifiers). All user-facing display strings updated. v0.5b (PermissionService) and v0.5c (onboarding wiring) follow. |
...
| **v0.5e — release APK + on-device verification** | _pending user's hands-on step_ | _tbd_ | _tbd_ | The user runs `flutter build appbundle --release` + `adb uninstall com.common_games.streak` + `adb install build/app/outputs/flutter-apk/app-release.apk` + the seven-step on-device verification on a real SM-S918B device. The release APK is the install boundary (applicationId changed). v0.5e is the right-side gate; v0.5f is the sign-off commit. No code lands in v0.5e — the user is the executor. |
