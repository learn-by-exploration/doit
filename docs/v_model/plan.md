...
| v1.8-pr-d / PR-D (2026-07-08) | SHIPPED | Rest-day caption wording flip (`X/Y rest days left` → `Used X of Y this month`) + localized empty-state body (`homeEmptyBody` ARB key). 2 screen-file edits (`home.dart` `_BudgetCaption.build` + the home empty-state call site) + 4 ARB keys (en + es mirror) + 5 NEW tests + 4 updated existing tests. 2084 → 2089 (+5 net). 3-gate green. APK discipline anchor maintained. SYS-193 / ADR-124 / WF-120. |
| v1.8-pr-c / PR-C (2026-07-08) | SHIPPED | Passphrase input on the Restore-from-Backup screen (C2 unblock v2/v3 encrypted-backup restore UX). 1 screen-file edit (`settings_restore.dart` — `TextEditingController` + obscure toggle + `SingleChildScrollView` wrap + `passphrase:` arg threaded through to `BackupService.importFrom`) + 6 NEW ARB keys (en + es: passphrase label / hint / show / hide / wrong-passphrase error / encrypted-backup hint) + regen `lib/l10n/gen/app_localizations*.dart` + 1 NEW test file (`test/screens/settings_restore_passphrase_test.dart`, 10 tests in 5 groups) + 1 EDIT test file (`test/screens/settings_restore_test.dart` — switched `_wrap()` to `support.localizedApp(...)` per the PR11 lesson). 2089 → 2099 (+10 net; matches the plan's "+10" target exactly). 3-gate green. APK discipline anchor maintained. SYS-192 / ADR-123 / WF-119. |
| v1.8-pr-b / PR-B (2026-07-08) | SHIPPED | Post-onboarding CoachMark tour + home empty-state "Show me around" CTA (C1 first-time-user onboarding — closes the "no guided tour of the FAB" + "no CTA on the home empty state" product gaps surfaced by the v1.8 UI consolidation sprint's user-testing pass). NEW `lib/ui/coach_mark.dart` (~280 LOC; 17th `lib/ui/` file) — `CoachMarkOverlay` primitive (`Stack` + scrim + `CustomPaint` ring around the target's RenderBox + Card callout with title + body + Next + Skip; canonical `ValueKey('tour.skip')` / `ValueKey('tour.next')`; `Semantics(liveRegion: true, label: tourBubbleAriaLabel)` for TalkBack) + `CoachMarkController.start(context, steps)` static helper walks N `TourStep`s in order (pops the route with `true` to advance, `false` to abort) + `TourStep.onAdvance: Future<void> Function()?` async hook for inter-step navigation (step 1 fires `Navigator.push(AddHabitScreen)` BEFORE step 2's `findRenderObject()` rect lookup). 1 EDIT service file (`lib/services/settings_service.dart` — new `ValueNotifier<bool> tourSeen` + `static const String _kTourSeenKey = 'doit.tour.seen'` + read in `init()` + `markTourSeen()` setter + reset in `resetForTesting()`). 2 EDIT screen files (`lib/screens/home.dart` — `_fabKey: GlobalKey(debugLabel: 'home.fab')` + `_AddFab(fabKey:)` param with `ValueKey('home.fab')` fallback + new private `_startTour(BuildContext)` method building the 2-step queue + `EmptyState` wrapped in `ValueListenableBuilder<bool>` with `PrimaryButton(key: ValueKey('home.empty.tour_cta'), label: Text(l.homeEmptyTourCta), ...)` rendered only when `tourSeen == false`; `lib/screens/add_habit.dart` — schedule-type `SegmentedButton<String>` carries `key: const GlobalObjectKey('tour.schedule_picker')`). 9 NEW ARB keys (en + es: `homeEmptyTourCta` / `tourStep1Title` / `tourStep1Body` / `tourStep2Title` / `tourStep2Body` / `tourNext` / `tourSkip` / `tourDone` / `tourBubbleAriaLabel` — the latter with `placeholders` metadata pinning `(String title, String body)` arg order) + regen `lib/l10n/gen/app_localizations*.dart`. 3 NEW test files (`test/ui/coach_mark_test.dart`, 9 tests in 4 groups: required content / CTAs / a11y liveRegion / multi-step controller; `test/services/settings_service_tour_flag_test.dart`, 4 tests: default / markTourSeen / persistence-survives-restart / resetForTesting clears; `test/screens/home_empty_state_tour_cta_test.dart`, 4 tests: CTA renders when tourSeen=false / CTA hides when tourSeen=true / Spanish locale / tap CTA mounts CoachMarkOverlay). Test count 2099 → 2116 (+17 net; +6 over the plan's "+11" target — the multi-step controller group + the CTAs group got more tests than planned). 3-gate green. APK discipline anchor maintained. SYS-191 / ADR-122 / WF-118. |

### v1.8-pr-e1 / PR-E1 — `scheduled_messages` Drift table for one-shot contact reminders (data-layer only)

PR-E1 is the data-layer half of the v2.0 retention "remind me to message this person at this exact time" feature. The feature itself spans two PRs — PR-E1 pins the schema shape (no UI, no service, no ARB, no platform changes) and PR-E2 wires the `url_launcher` integration + the Schedule-a-message screen + the notification tap-to-launch path. Splitting the data-layer migration into its own PR is canonical for this codebase (per `.claude/rules/lib-services.md` §"When changing this folder" / "A DB migration is its own PR. Do not bundle with a feature."); the split keeps each PR independently rollback-able and keeps the migration contract auditable in isolation.

**What landed.** NEW table class `ScheduledMessages` in `lib/services/db/tables.dart` (9 columns: `id` (TEXT PK) / `person_id` (TEXT nullable — a scheduled message MAY exist without a saved person, e.g., the user pasted a phone number and picked a fire time without saving a Person first) / `channel_tag` (TEXT — one of the 5 `PersonChannel` variant tags: 'dialer' | 'whatsapp' | 'telegram' | 'signal' | 'sms', matching the `People.channel` column text values verbatim) / `channel_handle` (TEXT — phone number E.164 with leading `+` for dialer/whatsapp/signal/sms, or `@`-stripped username for telegram; denormalized from `People.handle` at schedule time so the launch path works after a person delete) / `message_body` (TEXT nullable — null = user types in the chat app; non-null = pre-fill via the URI scheme's `?text=` param) / `fire_at_millis` (INTEGER — absolute epoch ms the reminder should fire) / `status` (TEXT — SQL default 'pending' via `withDefault(const Constant('pending'))`; lifecycle values are 'pending' | 'fired' | 'dismissed' | 'cancelled') / `created_at_millis` (INTEGER) / `fired_at_millis` (INTEGER nullable — audit timestamp set when the notification actually goes out); `@DataClassName('ScheduledMessageRow')`). NEW migration `lib/services/db/migrations/v5_to_v6.dart` (a single `m.createTable(db.scheduledMessages)` call — the simplest possible migration; the table is additive, no existing column is touched). EDIT `lib/services/db/schema.dart` (bump `kCurrentSchemaVersion` 5 → 6; add `ScheduledMessages` to the `@DriftDatabase(tables: [...])` list; add `if (from < 6) await migrateV5ToV6(m, this);` to `MigrationStrategy.onUpgrade`; add the v6 entry to the file-header version history). REGEN `lib/services/db/schema.g.dart` via `dart run build_runner build` (emits `$ScheduledMessagesTable extends ScheduledMessages` + `class ScheduledMessageRow extends DataClass` + `ScheduledMessagesCompanion.insert(...)` factory).

**What was tested.** NEW `test/db/migration_v5_to_v6_test.dart` (7 tests in 3 groups: schemaVersion/table-exists/columns [3 — `kCurrentSchemaVersion == 6` + `db.schemaVersion == 6`, the table exists in `sqlite_master`, all 9 expected columns are present with no spurious extras] / SQL-default-via-customInsert + nullable-NULL [2 — the `status` SQL default 'pending' applies when `ScheduledMessagesCompanion.insert` omits the column; nullable columns (`personId` / `messageBody` / `firedAtMillis`) accept NULL cleanly] / full-row-round-trip + lifecycle-storable [2 — a fully-populated row round-trips byte-for-byte with column-fidelity assertions on every field; all 4 status lifecycle values are storable as plain strings via 4 insert-then-select assertions]). 4 EDIT pre-existing test files (`migration_test.dart` + `migration_v2_to_v3_test.dart` + `migration_v3_to_v4_test.dart` + `migration_v4_to_v5_test.dart` — bump version-pin assertions from `5` to `kCurrentSchemaVersion` / `6`; the new `migration_v3_to_v4_test.dart` + `migration_v4_to_v5_test.dart` + `migration_v5_to_v6_test.dart` use `kCurrentSchemaVersion` to be future-proof against the next bump).

**Result.** Test count 2116 → 2123 (+7 net; matches +7 plan target exactly). 3-gate green (`dart format --output=none --set-exit-if-changed .` clean — 2 files auto-formatted on first pass, 0 on the verify pass; `flutter analyze --fatal-infos lib test` 0 issues after fixing the 4 first-pass issues — `isNull` ambiguous import between `package:drift` and `package:flutter_test` resolved by `import 'package:drift/drift.dart' hide isNull;` then dropping the drift import entirely when it turned out unused; `status: 'pending'` + `DateTime(2026, 7, 9, 10, 0).millisecondsSinceEpoch` both flagged `avoid_redundant_argument_values` — the first fixed by switching to `ScheduledMessagesCompanion.insert(...)` to exercise the SQL default, the second fixed by `DateTime(2026, 7, 9, 10).millisecondsSinceEpoch` which produces the same value with the implicit 0-minute default; the 3 pre-existing test failures in `migration_test.dart` + `migration_v2_to_v3_test.dart` were version-pin drift from the 5→6 bump, fixed by updating the assertions). **APK discipline anchor MAINTAINED** — no `AndroidManifest.xml` change, no `pubspec.yaml` change, no Kotlin change; the Drift migration is the entirety of the change surface. The schema bump ripples into 5 pre-existing test files, all fixed in this PR. SYS-194 / ADR-125 / WF-121.

**Next:** PR-E2 (url_launcher wire + `PersonChannel.launch()` method + Person-tile "Send message" CTA + Schedule-a-message screen + notification tap-to-launch — the second half of the "remind me to message this person at this exact time" feature; uses the `scheduled_messages` table this PR created).
