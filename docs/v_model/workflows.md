...
| WF-118 | v1.8-pr-b / PR-B | Post-onboarding coach-mark tour + home empty-state "Show me around" CTA (C1 first-time-user onboarding) — `CoachMarkOverlay` (scrim + Positioned ring around target's RenderBox + Card callout with title + body + Skip + Next/Skip with canonical `ValueKey('tour.skip')` / `ValueKey('tour.next')` + Semantics(liveRegion: true, label: tourBubbleAriaLabel) for TalkBack) + `CoachMarkController.start` walks N steps in order (pops with `true` to advance, `false` to abort) + `TourStep.onAdvance` async hook for inter-step navigation + `GlobalObjectKey('tour.schedule_picker')` cross-screen shared key + new SharedPreferences key `doit.tour.seen` (`ValueNotifier<bool> tourSeen` + `markTourSeen()` setter) + home empty-state `ValueListenableBuilder<bool>` wraps `EmptyState` with a `PrimaryButton(label: Text(l.homeEmptyTourCta), ...)` rendered only when `tourSeen == false`. | NEW `lib/ui/coach_mark.dart` (~280 LOC; 17th `lib/ui/` file) + EDIT `lib/services/settings_service.dart` (added `tourSeen` + `_kTourSeenKey = 'doit.tour.seen'` + read in `init()` + `markTourSeen()` + reset in `resetForTesting()`) + EDIT `lib/screens/home.dart` (added `_fabKey: GlobalKey(debugLabel: 'home.fab')`; `_AddFab` private widget passes `fabKey` to `AddFab` with `ValueKey('home.fab')` fallback; new `_startTour(BuildContext)` private method builds the 2-step queue — step 1 `onAdvance` pushes `AddHabitScreen`, step 2 targets `GlobalObjectKey('tour.schedule_picker')`; `EmptyState` wrapped in `ValueListenableBuilder<bool>(valueListenable: SettingsService.instance.tourSeen, ...)` with `PrimaryButton(key: ValueKey('home.empty.tour_cta'), ...)` when `tourSeen == false`) + EDIT `lib/screens/add_habit.dart` (the schedule-type `SegmentedButton<String>` carries `key: const GlobalObjectKey('tour.schedule_picker')`) + EDIT `lib/l10n/app_en.arb` (9 new keys: `homeEmptyTourCta` / `tourStep1Title` / `tourStep1Body` / `tourStep2Title` / `tourStep2Body` / `tourNext` / `tourSkip` / `tourDone` / `tourBubbleAriaLabel` with `placeholders` metadata pinning `(String title, String body)` arg order) + EDIT `lib/l10n/app_es.arb` (Spanish mirror) + regen `lib/l10n/gen/app_localizations*.dart` + NEW `test/ui/coach_mark_test.dart` (9 tests in 4 groups: required content / CTAs / a11y liveRegion / multi-step controller) + NEW `test/services/settings_service_tour_flag_test.dart` (4 tests: default / markTourSeen / persistence-survives-restart / resetForTesting clears) + NEW `test/screens/home_empty_state_tour_cta_test.dart` (4 tests: CTA renders when tourSeen=false / CTA hides when tourSeen=true / Spanish locale / tap CTA mounts CoachMarkOverlay). 2099 → 2116 (+17 net; +6 vs the plan's "+11" target — the multi-step controller group + the CTAs group got more tests than planned). 3-gate green. APK discipline anchor maintained. SYS-191 / ADR-122. |

| WF-122 | v1.8-pr-e2 / PR-E2 | Calling-contact feature wire (3-part split: SYS-195 wire + SYS-196 service + SYS-197 UI; SYS-198 closeout tests). (a) `url_launcher: ^6.3.x` pubspec dep + `PersonChannel.launch({String? body}) → Uri` on the sealed class (5 overrides — Dialer / WhatsApp / Telegram / Signal / Sms; body URL-encoded via `Uri.encodeComponent`; E.164 normalization at launch time) + `PersonChannel.fromTag(String, String)` static factory for Drift TEXT→typed rebuild (throws `ArgumentError` on unknown tag). (b) Android 11+ `<queries>` block (3 `<package>` for WhatsApp/Telegram/Signal + 3 `<intent>` for `ACTION_SENDTO sms:` / `ACTION_DIAL tel:` / `ACTION_VIEW https:`). (c) Kotlin `ReminderBridge.showNotification(... tapUri)` → `MainActivity.buildReminderNotification(... tapUri)` → `uriPendingIntent(ctx, alarmId, uriString)` scheme-switched to `Intent(ACTION_DIAL/SENDTO/VIEW, uri)` with `FLAG_ACTIVITY_NEW_TASK`. (d) `ScheduledMessageRepository` singleton (CRUD + status-machine; `ScheduledMessageStatus` enum + `fromWire` forward-compat; `_fromRow` Drift mapper). (e) `AlarmScheduler.scheduleScheduledMessage` + `cancelScheduledMessage` + `FakeAlarmScheduler` impl + production impl. (f) `ReminderService.onFireAlarm` scheduled-message branch — mark fired + build `ReminderEvent(tapUri: ...)` + `notifications.show(event)`. (g) `AddScheduledMessageScreen` (4-section form: Recipient / Channel / When / Message; Save disabled until recipient + future-time; re-routes through `PersonChannel.fromTag`). (h) `ScheduledMessagesListScreen` (pending + history `FutureBuilder`s; cancel `AppIconButton`). (i) AddPerson Call+Schedule CTAs in edit mode (visible when `_pickedPhone != null`; `_callNow` uses `ChannelDialer.launch` + `launchUrl` + SnackBar fallback; `_scheduleMessage` navigates to `AddScheduledMessageScreen(personId: ...)`). (j) Settings tile deep-linking to `ScheduledMessagesListScreen`. (k) Widget tests: 5 in `add_scheduled_message_test.dart` + 4 in `scheduled_messages_list_test.dart` + 2 in `add_person_calling_cta_test.dart` + extended 3 existing stubs to match the abstract `tapUri` parameter + 2 new tapUri passthrough tests in `platform_notification_service_test.dart` + 3 new scheduled-message tests in `reminder_service_test.dart` (whatsapp / sms / dialer / null-body). | EDIT `pubspec.yaml` (1 dep) + EDIT `pubspec.lock` (lockfile auto) + EDIT `lib/people/person.dart` (~80 LOC: `launch()` + `fromTag()`) + EDIT `android/app/src/main/AndroidManifest.xml` (~15 LOC: `<queries>` block) + EDIT `android/app/src/main/kotlin/com/doit/ReminderChannelProxy.kt` (~6 LOC: `tapUri` pass-through) + EDIT `android/app/src/main/kotlin/com/doit/MainActivity.kt` (~25 LOC: `uriPendingIntent` helper + builder branch) + NEW `lib/services/scheduled_message_repository.dart` (~340 LOC) + EDIT `lib/reminders/alarm_scheduler.dart` (~30 LOC: abstract + Fake + production impls) + EDIT `lib/services/reminder_service.dart` (~40 LOC: scheduled-message branch) + NEW `lib/screens/add_scheduled_message.dart` (~330 LOC) + NEW `lib/screens/scheduled_messages_list.dart` (~180 LOC) + EDIT `lib/screens/add_person.dart` (~40 LOC: CTAs + helpers) + EDIT `lib/screens/settings.dart` (~15 LOC: tile) + NEW `test/services/scheduled_message_repository_test.dart` (6 tests) + NEW `test/services/platform_notification_service_test.dart` (+2 tests) + EDIT `test/services/reminder_service_test.dart` (+4 tests + 3 scheduled-message tests) + EDIT `test/reminders/alarm_scheduler_test.dart` (+4 tests) + EDIT `test/services/reliability_service_test.dart` (stub update) + EDIT `test/reminders/doze_simulation_test.dart` (stub update) + EDIT `test/widgets/permission_sheet_test.dart` (stub update) + NEW `test/screens/add_scheduled_message_test.dart` (5 tests) + NEW `test/screens/scheduled_messages_list_test.dart` (4 tests) + NEW `test/screens/add_person_calling_cta_test.dart` (2 tests) + NEW `test/people/person_launch_test.dart` (7 fromTag + 5 launch + 7 round-trip tests = ~19 tests; the launch tests pin Dialer → `tel:`, WhatsApp → `https://wa.me/.../?text=...` with URL-encoded body, Telegram strips `@` and returns `https://t.me/handle?text=...`, Signal → `https://signal.me/#eu/...` (no body), Sms → `sms:...?body=...`). 2123 → 2198 (+75 net, +11 above the +64 plan target). 3-gate green. APK discipline anchor BUMPED per ADR-126. SYS-195..198 / ADR-126 / WF-122. |
